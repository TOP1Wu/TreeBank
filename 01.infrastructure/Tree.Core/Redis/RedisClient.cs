...
namespace Tree.Core.Redis
{
    public class RedisClient
    {
        ...
        public RedisClient(int dbNum = 2)
            : this(dbNum, null)
        {
        }
        ...
        public RedisClient(int dbNum, string readWriteHosts)
        {
            ...
            _conn = string.IsNullOrWhiteSpace(readWriteHosts)
                ...
                : RedisClientManager.GetConnectionMultiplexer(readWriteHosts);
        }
        ...
        {
            var prefixKey = CustomKey ?? RedisClientManager.SysCustomKey;
        ...
        {
            List<KeyValuePair<RedisKey, RedisValue>> valuePairs =
                keyValues.Select(p => new KeyValuePair<RedisKey, RedisValue>(AddSysCustomKey(p.Key), p.Value)).ToList();
            return Do(db => db.StringSet(valuePairs.ToArray()));
        }
        ...
        /// <returns></returns>
        public T LockWait<T>(string key, Func<T> getDataFunc, Action<T> setDataAction = null, TimeSpan? duration = null,
            TimeSpan? lockTimeout = null) where T : class
        {
            ...
            return Do(db =>
            {
                ...
                while (true)
                {
                    if (!db.LockTake(key, token, duration ?? TimeSpan.FromSeconds(10)))
                    {
                        ...
                        if (DateTime.UtcNow.Subtract(now) > lockTimeout)
                        {
